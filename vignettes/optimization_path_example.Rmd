...
proxy) and any covariates to split on. ROOT searches over tree-structured
weight functions to minimize the supplied objective, then returns a Rashomon
set of near-optimal trees and a single summary tree characterizing the final
weight assignments.
...
Constructing a minimum-variance portfolio is a classic optimization problem.
The standard approach (Markowitz mean-variance optimization) produces weights
that are continuous and can be hard to interpret or communicate. ROOT offers a
complementary perspective: it returns a **binary inclusion rule** — a sparse
decision tree that describes, in plain language, which types of assets belong
in the low-variance portfolio.

### Mapping to ROOT's framework

ROOT minimizes the objective:

$$
L(w, D) = \sqrt{\frac{\sum_i w_i \cdot v_i^2}{\left(\sum_i w_i\right)^2}}
$$

where $v_i^2$ (`vsq`) is a per-unit variance proxy. In the portfolio context
we set `vsq` to the historical return variance of each asset. ROOT then finds
the binary weight $w(X) \in \{0, 1\}$ — described as a decision tree over
asset features such as beta and volatility — that minimizes this quantity.
...
plot(
  ...
  pch  = 16, col = "#4E79A7AA",
  main = "Asset universe: beta vs volatility"
)
```
...
- **`beta >= 1.7` and `vol < 0.33`**: include the asset despite high market
  sensitivity, because idiosyncratic risk is controlled (8 assets).
- **`beta >= 1.7` and `vol >= 0.33`**: exclude the asset — high market
  sensitivity combined with high idiosyncratic volatility drives up portfolio
...
plot(
  ...
  main = "Portfolio inclusion decisions (● included, ✕ excluded)"
)
legend(
  "topleft",
...
and the resulting characterized tree remains fully interpretable.
