...
broader target population. If certain subgroups are systematically
underrepresented in the trial, estimates of the **Target Average Treatment
Effect (TATE)** can be imprecise or misleading when transported to that
population.

**ROOT** addresses this by learning an interpretable binary weight function
$w(X) \in \{0, 1\}$ that identifies the *well-represented* subpopulation —
the subset of the target population for which the trial provides reliable
evidence. Units are included ($w = 1$) or excluded ($w = 0$) according to
tree-structured rules that minimize the variance of the transported treatment
effect estimator.

The resulting estimand is the **Weighted Target Average Treatment Effect
(WTATE)**: the average treatment effect restricted to the represented
subpopulation, estimated with lower variance than the unweighted SATE.
...
study. It contains 2,000 individuals split between an RCT sample and a target
population.
...
1. Estimates transportability scores using logistic regression models for
...
more reliable evidence. A smaller SE for the WTATE relative to the SATE
...
giving a single interpretable rule that is robust to the specific
randomness of any individual tree.
